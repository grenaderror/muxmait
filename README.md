...
- `-g`, `--git`: Git commit helper. Captures `git status` and `git diff` (or accepts piped input), skips tmux screen capture, and prompts for `git add <files>; git commit -m "..."; git push`. Defaults to `openrouter/qwen/qwen3.8-27b:free` (shorthand `q38f`), then `openrouter/nvidia/nemotron-3.5-lightning:free` (`nlf`), several Gemini models, and finally `openrouter/free` (`orf`). The whole fallback chain is capped at 10 seconds: a model that hasn't answered in its slice is abandoned for the next one. A model passed explicitly with `-m` is tried first.
